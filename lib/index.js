...
window.addEventListener('load', function () {
  ...
  myZone.style.border = '1px solid black'; // Append the new div to the body of the document
...
function useKeyboardInput(inputManager, userKeyMap) {
  ...
  manager?.on("move", function (evt, data) {
    ...
    } else if (data?.direction?.y === "up" && data?.angle.degree > 45 && data?.angle.degree < 135 && data?.vector.y > 0.99) {
      setInputsPressed(prevState => ({ ...prevState,
        run: true,
        up: false,
        down: false
